...
public class SMSender {
    ...
    public SMSender(Service service){
    ...
    public void sendSMS(final String phoneNumber, String message){
        ...
        PendingIntent sentPI = PendingIntent.getBroadcast(service, 0, new Intent(SENT), 0|PendingIntent.FLAG_IMMUTABLE);
        PendingIntent deliveredPI = PendingIntent.getBroadcast(service, 0, new Intent(DELIVERED), 0|PendingIntent.FLAG_IMMUTABLE);
    ...
    private void sendNextMessage(String message){
        if(thereAreSmsToSend()){
            ...
            Toast.makeText(service, "SMS 전송 완료하였습니다.",
            ...
            public void onReceive(Context context, Intent intent) {
                switch (getResultCode()) {
                    case Activity.RESULT_OK:
                        ...
                        Toast.makeText(service, "SMS sent",
                                Toast.LENGTH_SHORT).show();
                        break;
                    case SmsManager.RESULT_ERROR_GENERIC_FAILURE:
                        Toast.makeText(service, "Generic failure",
                                Toast.LENGTH_SHORT).show();
                        break;
                    case SmsManager.RESULT_ERROR_NO_SERVICE:
                        Toast.makeText(service, "No service",
                                Toast.LENGTH_SHORT).show();
                        break;
                    case SmsManager.RESULT_ERROR_NULL_PDU:
                        Toast.makeText(service, "Null PDU",
                                Toast.LENGTH_SHORT).show();
                        break;
                    case SmsManager.RESULT_ERROR_RADIO_OFF:
                        Toast.makeText(service, "Radio off",
                                Toast.LENGTH_SHORT).show();
                        break;
                }
            }
    ...
    public BroadcastReceiver getDeliveredBroadCast(){
        return new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                switch (getResultCode()){
                    case Activity.RESULT_OK:
                        Toast.makeText(service.getBaseContext(), "SMS DELIVERED", Toast.LENGTH_SHORT).show();
                        break;
                    case Activity.RESULT_CANCELED:
                        Toast.makeText(service.getBaseContext(), "SMS not delivered", Toast.LENGTH_SHORT).show();
                        break;
                }
            }
        };
